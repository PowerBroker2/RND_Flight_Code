...
void rollLimiter()
{
  if (limitRoll)
  {
    if (plane.roll > MAX_ROLL_RIGHT)
    {
      int tempRollPulseLen = (ROLL_LIMIT_SCALE * (plane.roll - MAX_ROLL_RIGHT)) + MS_MID;

      if (tempRollPulseLen > rollPulseLen)
        rollPulseLen = tempRollPulseLen;
    }
    else if (plane.roll < MAX_ROLL_LEFT)
    {
      int tempRollPulseLen = (ROLL_LIMIT_SCALE * (plane.roll - MAX_ROLL_LEFT)) + MS_MID;
...
void handleControllers()
{
  ...
  else // manual
  {
    ...
    flapServo.writeMicroseconds(flapPulseLen);
  }
}
