...
TEST_FILE_NUM = 1
TEST_FILE     = os.path.join(SCRIPT_DIR, 'test_data', 'flight_{}.txt'.format(TEST_FILE_NUM))
...
def setup_args():
    ...
    parser.add_argument('--dn',   help='flight number - used with serial download only')
...
def plot_data(data_file, plot_title='Flight Data'):
    df = pd.read_csv(data_file)
    import pprint
    pprint.pprint(df)
    df.plot(subplots=True, figsize=(8, 8), x='epoch_ms', title=plot_title)
    plt.legend(loc='best')
...
def flights_from_ser(connection):
    connection.write('ls\n'.encode())
        ...
        if args.ser.lower() == 'true':
            if args.port and args.baud:
                try:
                    ...
                    flight_files = flights_from_ser(connection)
                    ...
                    if args.dn:
                        flight_number = int(args.dn)
                        flight = 'flight_{}.txt'.format(flight_number)
                        
                        if flight in flight_files:
                            print(flight)
                            ...
                            if os.path.exists(SER_DATA_PATH):
                                try:
                                    plot_data(SER_DATA_PATH, flight)
                            ...
                            print('ERROR - flight file {} not found on SD'.format(flight))
